...
class ThreadDisplay extends Component{
    ...
    componentDidMount(){
        axios.get(`/backend/threads/${this.props.threadid}`).then(res=>{
            this.setState({
    ...
    upvote = () =>{
        ...
        axios.put(`/backend/threads/karma/${this.state.threadData._id}`,{vote:vote,user:this.props.username,type:'upvote'})
            .then(res=>{
                this.setState({
                    threadData:res.data.thread,
    ...
    downvote = () =>{
        ...
        axios.put(`/backend/threads/karma/${this.state.threadData._id}`,{vote:vote,user:this.props.username,type:'downvote'})
            .then(res=>{
                this.setState({
                    threadData:res.data.thread,
                    positive:false,
        ...
        return(
            <div className='threadContainer' style ={{background: "#ED4832"}}> {/*053C5E */}
                ...
                <div className='rightDiv'style ={{padding: "10px"}}>
                    ...
                    <span>created by <Link style = {{color: "white"}} to={`/users/${this.state.threadData.author}`}>{this.state.threadData.author}</Link> </span> <Moment fromNow>{this.state.threadData.created}</Moment>
                    <Divider horizontal></Divider>
                    <Message className='messagebody'>{this.state.threadData.body}</Message>
                    {this.props.username === this.state.threadData.author && <Button onClick={this.toggleEdit} >Edit</Button>}
                    {this.props.username === this.state.threadData.author && <Button onClick={this.delete} >Delete</Button>}
                </div>
            </div>
        )
    }
